...
void correctConditions(int n, condition *list, int *indices) {
  ...
  for (int i = 0; i < n - 1; i++) {
    ...
    for (int j = i; j < n; j++)
      // Si la condici�n actual corresponde a un nodo posterior al nodo
      // eliminado por aplicar la condici�n anterior, se debe actualizar su
      // posici�n.
...
void fusionDirichlet(int n1, condition *list1, int n2, condition *list2, int n3,
                     condition *list3, condition *list) {
  int index = 0;
...
void leerMallayCondiciones(mesh &m, char *filename) {
  ...
  fusionDirichlet(ndirich_u, dirichlet_u, ndirich_v, dirichlet_v, ndirich_p,
                  dirichlet_p, m.getDirichlet());

  correctConditions(ndirich_u + ndirich_v + ndirich_p, m.getDirichlet(),
                    m.getDirichletIndices());
}
