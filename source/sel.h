node selectNode(int i, element e, mesh &m) {
  ...
  switch (i) {
    case 1:
      n = m.getNode(e.getNode1() - 1);
      break;
    case 2:
      n = m.getNode(e.getNode2() - 1);
      break;
    case 3:
      n = m.getNode(e.getNode3() - 1);
      break;
  }
  return n;
}
...
float calculateLocalD(int i, mesh m) {
  ...
  matriz.push_back(row1);
  matriz.push_back(row2);
...
void calculateLocalA(int i, Matrix &A, mesh m) {
  ...
  A.at(0).at(0) = calcularTenedor(e, YE, 3, 1, m);
  A.at(0).at(1) = calcularTenedor(e, YE, 1, 2, m);
  A.at(1).at(0) = calcularTenedor(e, EQUIS, 1, 3, m);
  A.at(1).at(1) = calcularTenedor(e, EQUIS, 2, 1, m);
}
...
Matrix createLocalK(int e, mesh &m) {
  ...
  float tau, kappa, sigma, lambda, Ae, J, D;

  // Componentes de K
  // [ A+K  G ]
  // [  D   0 ]
  Matrix matrixA, matrixG, matrixL, matrixD, matrixE;
  Matrix K, Gamma, Gammat, Alpha, Beta, Alphat, Betat, BPrima, BPrimat;
  ...
  calculateGammaMatrix(Gamma);
  calculateLocalA(e, Alpha, m);
  calculateBetaMatrix(Beta);
  productRealMatrix(
      tau * J / (24 * D),
      productMatrixMatrix(Gamma, productMatrixMatrix(Alpha, Beta, 2, 2, 6), 6,
                          2, 6),
      matrixA);
  ...
  productRealMatrix(
      ...
      productMatrixMatrix(
          Betat,
          productMatrixMatrix(Alphat, productMatrixMatrix(Alpha, Beta, 2, 2, 6),
                              2, 2, 6),
          6, 2, 6),
      matrixG);

  // Calculo Bprima y el producto gamma x alpha y Bprima porque se repetite
  calculateBPrima(BPrima);
  Matrix LandDHelper = productMatrixMatrix(
      Gamma, productMatrixMatrix(Alpha, BPrima, 2, 2, 3), 6, 2, 3);
  // Preparando matrixL
  sigma = m.getParameter(SIGMA);
  calculateBPrima(BPrima);
  productRealMatrix((J * sigma) / (48 * D), LandDHelper, matrixL);
  // Preparando matrixD
  lambda = m.getParameter(LAMBDA);
  calculateBPrima(BPrima);
  productRealMatrix((2 * J * lambda) / (18 * D), LandDHelper, matrixD);
  // Preparando matrixE
  transpose(BPrima, BPrimat);
  transpose(Gamma, Gammat);
  productRealMatrix(
      J / (6 * D),
      productMatrixMatrix(BPrimat, productMatrixMatrix(Alphat, Gammat, 2, 2, 6),
                          3, 2, 6),
      matrixE);
  ...
  ubicarSubMatriz(K, 0, 5, 6, 8,
                  sumMatrix(hacerNegativo(matrixL), matrixD, 6, 3));
...
Vector createLocalb(int e, mesh &m) {
  ...
  zeroes(b0, 6);
  productMatrixVector(Gamma, f, b0);
...
void assemblyK(element e, Matrix localK, Matrix &K, int nnodes) {
  ...
  int index4 = index1 + nnodes, index5 = index2 + nnodes,
      index6 = index3 + nnodes;
  int index7 = index1 + 2 * nnodes, index8 = index2 + 2 * nnodes,
      index9 = index3 + 2 * nnodes;
